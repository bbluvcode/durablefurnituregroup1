...
function Login() {
  ...
  const checkLogin = (userCheck) => {
    ...
    if (
      users.filter((user) => user.username === userCheck.username).length !== 0
    ) {
      if (
        users.filter((user) => user.username === userCheck.username)[0]
  ...
  function addUser(newUser) {
    console.log(newUser);
    if (
      arrUsername.findIndex((element) => element === newUser.username) !== -1
    ) {
  ...
  return (
    <div
      className={`container-login ${signIn ? null : "right-panel-active"}`}
      id="container-login"
    >
